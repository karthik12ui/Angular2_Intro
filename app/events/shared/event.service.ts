...
@Injectable()  // This is required when an service is injceting another service but nice to have
export class EventService{
    getEvents() :Observable<IEvent[]>{   //Method to return events 

      let subject = new Subject<IEvent[]>() // Rxjs observable
      setTimeout(()=> {subject.next(EVENTS); subject.complete();}, 100)  // After 2Sec we are adding Data to Stream
        return subject
    }
  ...
  updateEvent(event){
    let index = EVENTS.findIndex(x=>x.id = event.id)  // Find the Current Event and update in array
    EVENTS[index] = event
  }
...
const EVENTS:IEvent[] =[    //Array of list
    {
      id: 1,
